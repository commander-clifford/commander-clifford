<?php 
	...
	foreach($sizes as $size_name => $size_width){
		...
		//Save the file path into the database for the logged in user
		$query = "UPDATE users 
		...
		if($db->affected_rows >= 1):
			$statusmsg .= 'Your image was added to the database. ';
		else:
			$statusmsg .='There was a problem adding the image to the database';
		endif;	
	}
...
<form action="<?php $_SERVER['PHP_SELF'] ?>?page=profile" method="post" enctype="multipart/form-data">
	...
	<label for="uploadfile">Choose an Image(.jpg .gif .png)</label>
